...
export async function handleProjectsRequest(c: Context) {
  try {
    ...
    logger.api.info("Reading claude config from: {path}", {
    ...
    try {
      ...
      if (config.projects && typeof config.projects === "object") {
        ...
        logger.api.info("Found {count} projects in config", {
        ...
        for (const path of projectPaths) {
          ...
          logger.api.info("Project {path} -> {encoded}", {
            path,
            encoded: encodedName || "null",
          });
        ...
        logger.api.info("Returning {count} projects with history", {
